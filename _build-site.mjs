...
.subhead{font-family:'Plus Jakarta Sans',sans-serif;font-weight:700;font-size:14px;color:var(--mut);margin:4px 0 10px}
.subcats{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:8px;margin-bottom:24px}
.subcat{display:flex;justify-content:space-between;align-items:center;gap:10px;background:var(--panel2);border:1px solid var(--line);border-radius:10px;padding:11px 14px;cursor:pointer;font:inherit;color:var(--ink);text-align:left}
.subcat:hover{background:#fff;border-color:var(--acc);box-shadow:var(--shadow)}
.subcat.active{background:var(--acc);border-color:var(--acc);color:#fff}
.scn{font-weight:600;font-size:14px}.scc{font-size:12px;color:var(--mut);font-variant-numeric:tabular-nums}.subcat.active .scc{color:oklch(0.92 0.05 252)}
